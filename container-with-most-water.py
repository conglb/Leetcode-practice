class Queue:
    ...
    def second(self):
        if len(self.outbox) > 1:
            return self.outbox[len(self.outbox) - 2]
        else:
            return self.inbox[0]
    def last(self):
        if len(self.inbox) > 0:
            return self.inbox[len(self.inbox) - 1]
        else:
    ...
    def __next__(self):
        if self.index < len(self):
            if self.index < len(self.outbox):
                self.index += 1
                return self.outbox[len(self.outbox) - self.index]
            else:
                self.index += 1
                return self.inbox[self.index - 1 - len(self.inbox)]
        else:
            raise StopIteration
